...
function activate(context) {
  ...
  context.subscriptions.push(vscode.window.registerCustomEditorProvider('p2.tbf', {
    async resolveCustomTextEditor(document, panel) {
      ...
      webview.html = `<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src ${webview.cspSource}; script-src 'nonce-${nonce}';"><link rel="stylesheet" href="${resource('editor.css')}"></head><body>
<header><section class="metadata"><h1 id="filename">Editor TBF</h1><label>Diretório original<input id="directory" readonly></label><label>Arquivo original<input id="original" readonly></label><label>Contexto (objeto JSON)<textarea id="context" rows="2" spellcheck="false"></textarea></label></section>
